#include "ExcuseMode1.h"

cv::Rect MU(240, 30, 920, 910);
cv::Rect DepthTu(180, 120, 150, 150);
cv::Rect DepthTuX(500, 400, 100, 100);

void ExcuseMode1::excusemode1(TYcamera &a, RobotControl &b, chooseblack &c, complexIm &d)
{
    tu = MU;
    ...
    while (1)
    {
       ...
       // cv::Mat imageP = a.getGainImage(imageDepthP);
        ...
        d.getDepth(imageDepthP, needzp, DepthTu);
        if (needzp < MaxWorkZ)
        {
            needzp = -needzp + 2400;
            //needzp = needzp + 100;
        ...
        //cv::Mat ColorIm = a.getGainImage(DepthImage);

        cv::Mat color1 = ColorIm.clone();
        cv::Mat depth1 = DepthImage.clone();
        cv::Mat outputIm;
        ColorImage = ColorIm.clone();

        float NeedZ;

        d.getDepth(DepthImage, NeedZ, DepthTu);
        ...
        if (NeedZ < MaxWorkZ)
        {
            ...
            MainProcess(); Delay(50); MainProcess(); DWrite1R(0, 7001, 1); Delay(1000);
            ...
            if (NeedZ < 1900)
                d.compleximNight1(ColorIm, outputIm, cv::Size(), BinaryIms, tu, imcenters, Radiuses, NeedZ, NeedZ0);
                //d.complexim2ModeTry2(ColorIm, outputIm, cv::Size(), BinaryIms, tu, imcenters, Radiuses, NeedZ, NeedZ0);
            else
                d.compleximNight1(ColorIm, outputIm, cv::Size(), BinaryIms, tu, imcenters, Radiuses, NeedZ, NeedZ0);
              // d.complexim2ModeTry3(ColorIm, outputIm, cv::Size(), BinaryIms, tu, imcenters, Radiuses, NeedZ, NeedZ0);
           ...
           // d.GetEveryDepth(ROIDepthImage, imcenters, Radiuses, depthData);

            d.getRobDataMode2(NowLayerZ,NeedZ, NeedZ0, imcenters, Radiuses, Robx, Roby, Robz, R, Robwt1, Robwt2);
           // d.getRobData(NeedZ, NeedZ0, imcenters, Radiuses, Robx, Roby, Robz, tu, Robwt1, Robwt2);
           // d.getRobData2(NeedZ, NeedZ0, imcenters, Radiuses, Robx, Roby, Robz, tu, Robwt1, Robwt2, depthData);
...
void ExcuseMode1::Nowexcusemode1(TYcamera& a, RobotControl& b, chooseblack& c, complexIm& d)
{
    tu = MU;
    ...
    while (1)
    {
        ...
        d.getDepth(imageDepthP, FirstPaperZ, DepthTuX);

        while (FirstPaperZ == 0) { d.getDepth(imageDepthP, FirstPaperZ, DepthTuX); }
...
void ExcuseMode1::run()
{
    Nowexcusemode1(a,b,c,d);
}
